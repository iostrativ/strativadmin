-- =============================================
-- PHASE 1: CONVERT TO UNIFIED BLOCK SYSTEM (v2 - Idempotent)
-- This version is safe to run multiple times
-- It will delete existing system page sections before recreating them
-- =============================================

-- =============================================
-- STEP 0: CLEANUP (if migration was run before)
-- =============================================

-- Delete existing page_sections for system pages
DELETE FROM public.page_sections
WHERE page_id IN (SELECT id FROM public.pages WHERE is_system_page = true);

-- Delete existing system pages (will be recreated)
...
-- =============================================
-- STEP 8: SEED PORTFOLIO PAGE (Optional)
-- =============================================

DO $$
DECLARE
  v_portfolio_page_id UUID;
BEGIN
  SELECT id INTO v_portfolio_page_id FROM public.pages WHERE slug = 'portfolio';
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'subheadline', 'Explore our successful projects and see how we''ve helped businesses transform',
      'alignment', 'center'
    ),
    0,
    true
